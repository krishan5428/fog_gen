...
import 'package:fire_nex/presentation/dialog/add_number_fire.dart';
...
class FireNumbersPage extends StatefulWidget {
  const FireNumbersPage({super.key, required this.panelSimNumber});
...
class _FireNumbersPageState extends State<FireNumbersPage> {
  String? adminNumber;
  late Future<List<FireNumber>> _fireNumberFeature;
  ...
  void _loadNumbers() {
    _fireNumberFeature = context
        .read<FireNumberViewModel>()
        .getFireNumbersByPanelSimNumber(widget.panelSimNumber);
  }

  @override
  Widget build(BuildContext context) {
    final viewModel = context.read<FireNumberViewModel>();

    return Scaffold(
      appBar: CustomAppBar(pageName: 'Fire Numbers', isFilter: false),
      body: Column(
        children: [
          ...
          Expanded(
            child: FutureBuilder<List<FireNumber>>(
              future: _fireNumberFeature,
              builder: (context, snapshot) {
                  ...
                  itemBuilder: (context, index) {
                    if (index == numbers.length && numbers.length < 9) {
                      return Padding(
                        ...
                        child: CustomButton(
                          buttonText: 'ADD New Fire Number',
                          icon: Icons.add,
                          onPressed:
                              adminNumber == null
                                  ? null
                                  : () async {
                                    ...
                                    if (success == true) {
                                      ...
                                      ScaffoldMessenger.of(
                                        ...
                                        const SnackBar(
                                          content: Text(
                                            'New Fire Number Added successfully',
                                          ),
                                        ),
                                      );
                                    } else {
